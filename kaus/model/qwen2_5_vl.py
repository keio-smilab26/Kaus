...
class Qwen2_5_VLAttention(nn.Module):
    ...
    def forward(
        ...
        past_key_value: Optional[Tuple[torch.Tensor]] = None,  # [MODIFIED]
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        if cache_position is None:
            ...
            # )
            past_seen_tokens = (
                past_key_values[0][0].current_length
        ...
        for idx, decoder_layer in enumerate(self.layers):  # [MODIFIED]
            if output_hidden_states:
                all_hidden_states += (hidden_states,)

            past_key_value = past_key_values[idx]  # [MODIFIED]
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer(
                    ...
                    position_ids=position_ids,
                    past_key_value=past_key_value,  # [MODIFIED]
                    output_attentions=output_attentions,
                    use_cache=use_cache,
                    cache_position=cache_position,
                    position_embeddings=position_embeddings,
    ...
    def _update_causal_mask(
        ...
        # )
        past_seen_tokens = (
            past_key_values[0][0].current_length if past_key_values is not None else 0
        )  # [MODIFIED]
        ...
        # DynamicCache or no cache
            ...
            target_length = (
                ...
                else past_seen_tokens + sequence_length + 1  # No id why
            )
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config):
        config.max_position_embeddings = 4096  # [MODIFIED]
        super().__init__(config)
    ...
    def forward(
        ...
        if position_ids is None and (
            ...
            if (
                ...
                or (
                    past_key_values is None or past_key_values[0][0].current_length == 0
                )  # [MODIFIED]
            ):
...
if __name__ == "__main__":
    ...
    #     return_tensors="pt",
    # ).to(model.device)
    # output = model.generate(**inputs, use_cache=False, max_new_tokens=128)
